...
static ReplacedTextureFormat FromD3D9Format(u32 fmt) {
	switch (fmt) {
...
static D3DFORMAT ToD3D9Format(ReplacedTextureFormat fmt) {
	switch (fmt) {
...
void TextureCacheDX9::LoadTextureLevel(TexCacheEntry &entry, ReplacedTexture &replaced, int srcLevel, int dstLevel, int maxLevel, int scaleFactor, u32 dstFmt) {
	...
	if (replaced.GetSize(srcLevel, w, h)) {
		...
		CheckAlphaResult alphaResult = DecodeTextureLevel((u8 *)pixelData, decPitch, tfmt, clutformat, texaddr, srcLevel, bufw, false, false, false);
