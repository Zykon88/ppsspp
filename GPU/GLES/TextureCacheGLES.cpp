...
void TextureCacheGLES::LoadTextureLevel(TexCacheEntry &entry, ReplacedTexture &replaced, int srcLevel, int dstLevel, int scaleFactor, Draw::DataFormat dstFmt) {
	...
	if (replaced.GetSize(srcLevel, w, h)) {
		int bpp = replaced.Format(srcLevel) == ReplacedTextureFormat::F_8888 ? 4 : 2;
		decPitch = w * bpp;
		...
		dstFmt = ToDataFormat(replaced.Format(srcLevel));
	} else {
		...
		CheckAlphaResult alphaStatus = DecodeTextureLevel(pixelData, decPitch, GETextureFormat(entry.format), clutformat, texaddr, srcLevel, bufw, true, false, false);
